...
from data_loader import PathgraphomicFastDatasetLoader_AE_feaOnly, PathgraphomicDatasetLoader_AE_feaOnly
...
def train(opt, data, mask, device, k):
    ...
    ################################################
    # Testing: load second stage
    ################################################
    use_patch, roi_dir = ('_patch_', 'all_st_patches_512') if opt.use_patch == True else ('_', 'all_st')
    custom_data_loader = PathgraphomicDatasetLoader_AE_feaOnly(opt, data, mask, split='train', mode=opt.mode)
    ...
    for epoch in tqdm(range(opt.epoch_count, opt.niter + opt.niter_decay + 1)):
        ...
        if epochs_cnt > start_early_stop + 1:
            ...
            if num_epoch_no_improvement == patience_epoch:
                ...
                print(epochs_cnt)
                break
    return model, optimizer, metric_logger #, best_eval_metric_value
...
def test(opt, model, data, mask, split, device):
    name_list = []
    model.eval()
    custom_data_loader = PathgraphomicFastDatasetLoader_AE_feaOnly(opt, data, mask, split,
                                                                   mode=opt.mode)
...
def test_missingModa(opt, model, data, mask, split, device):
    model.eval()
    custom_data_loader = PathgraphomicDatasetLoader_AE_feaOnly(opt, data, mask, split,
                                                        mode=opt.mode)
